...
#define COMBO_IOC_ANT_IC_HW_VERSION			7
#define COMBO_IOC_ANT_IC_FW_VERSION			8
...
//	 ANT_DEBUG_V("Set ANT Tx Value\n");
     ...
     if (1 == dataType)
 	 	status = ant_tx_message_flowcontrol_none(SINGLE_CHANNEL, txMessageLength, pSendBuf);	
     else
	 	status = ant_tx_message_flowcontrol_wait(SINGLE_CHANNEL, SINGLE_CHANNEL, txMessageLength, pSendBuf);
...
1) detect HW version
2) download right frimware based on HW version

*/
static int download_ANT_firmware(ant_channel_info_t *pstChnlInfo)
{
	ANT_FUNC_START();
	int eChannel;
	unsigned int hw_ver, fw_ver, chip_ver;
	int res;

	ANT_DEBUG_D("query hw version setup 1\n");
    if (ioctl(pstChnlInfo->iFd, COMBO_IOC_ANT_IC_HW_VERSION, &hw_ver) < 0)
    {
        ANT_ERROR("get COMBO_IOC_ANT_IC_HW_VERSION failed\n");
        return -1; 
    }
	ANT_DEBUG_D("query fw version setup 2\n");
    if (ioctl(pstChnlInfo->iFd, COMBO_IOC_ANT_IC_FW_VERSION, &fw_ver) < 0)
    {
        ANT_ERROR("get COMBO_IOC_ANT_IC_FW_VERSION failed\n");
        return -1; 
    }
	ANT_DEBUG_D("query version: hw: %d,fw: %d !\n",hw_ver,fw_ver);
    if((hw_ver == 0x8A00) && (fw_ver == 0x8A00))
    {
    	ANT_DEBUG_D("MT6630_E1\n");
		chip_ver = 1;
    }
    else if((hw_ver == 0x8A10) && (fw_ver == 0x8A10))
    {
    	ANT_DEBUG_D("MT6630_E2\n");
    	chip_ver = 2;
    }
    else if((hw_ver == 0x8A11) && (fw_ver == 0x8A10))
    {
    	ANT_DEBUG_D("MT6630_E2\n");
		chip_ver = 2;
    }		
    ...
    {
    	ANT_DEBUG_D("MT6630_E3\n");
		chip_ver = 2;
    } else {
    	ANT_ERROR("Unknown chip hw version,download default ram-code!\n");
    	chip_ver = 2;
    }

	res = ioctl(pstChnlInfo->iFd, COMBO_IOCTAL_ANT_DOWNLOAD_FIRMWARE, chip_ver);
	if(res == 0)
		ANT_DEBUG_D("download firmware ok\n");
	else
		ANT_ERROR("firmware download error\n");
...
int ant_enable(void)
{
   ...
   for (eChannel = 0; eChannel < NUM_ANT_CHANNELS; eChannel++) {
   		iRet = download_ANT_firmware(&stRxThreadInfo.astChannels[eChannel]); 
	  	if(iRet){
			ANT_ERROR("download_ANT_firmware failed!\n");
			goto out;
		}  
   }
