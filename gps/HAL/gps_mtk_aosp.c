...
void alarm_handler(){
	...
	if (MNL_AT_TEST_STATE != MNL_AT_TEST_UNKNOWN){
		...
		if(test_mode_flag)
		{
			...
			// In a weak GPS signal, If there is no CN measurement(alarming), null string should be reported using test_cb()
			gps_state_test_sms_airtest_no_result_report();
		}
	} 
}
	...
	{//(1 == MNL_AT_SIGNAL_MODE)){
		...
		if(time_diff >= hal_test_data.time_delay)
		{
			...
			if(gps_nmea_end_tag==1){
				int test_stopped = 0;
				DBG("Success_Num = %d, Prev_Success_Num = %d, Wait_Num = %d, Failure_Num = %d,Completed_Num = %d, Avg_CNo = %d, Dev_CNo = %d, Err_Code = %d, MNL_AT_TEST_STATE = %d", 
					  Success_Num, prev_success_num, Wait_Num, Failure_Num, Completed_Num, Avg_CNo, Dev_CNo, Err_Code, MNL_AT_TEST_STATE);
				if(Success_Num > 0){
...
if(sta.status == GPS_STATUS_ENGINE_ON || sta.status == GPS_STATUS_SESSION_BEGIN || MNL_AT_SIGNAL_MODE || MNL_AT_TEST_FLAG) {
    if(1 == status){
    ...
    switch(type) {
        ...
        case MNL_AGPS_CMD_AGPS_LOC: {//235
            ...
            if(callback_backup.location_cb){
                ...
                callback_backup.location_cb(&mlocation);
            }else{
		...
		case HAL_CMD_MEASUREMENT:{
            ...
            for (i = 0; i < 32; i++) {
                if (mtk_gps_measurement[i].PRN != 0) {
                    ...
                    gpsdata.measurements[num].accumulated_delta_range_uncertainty_m = mtk_gps_measurement[i].AcDRUnInMeters;
                    ...
                    gpsdata.measurements[num].pseudorange_rate_uncertainty_mps = mtk_gps_measurement[i].PRRateUnInMeterPreSec;
...
mtk_gps_init(GpsCallbacks* callbacks)
{
	...
	else if(strcmp(chip_id, "0x6630") == 0 || strcmp(chip_id, "0x6752") == 0)
	{
		gps_epo_type = 0;//G+G
	}
	else
...
static void *thread_epo_file_update(void* arg){
    ...
    while(1){
		...
		gps_download_epo_enable();
		//DBG("nw_connected =%d epo_download_failed = %d nw_type=%d,s->epo_data_updated =%d\n", nw_connected,epo_download_failed,nw_type,s->epo_data_updated);
...
//				time_t time_st;
		...
		//DBG("gps_epo_wifi_trigger =%d,started=%d\n", gps_epo_wifi_trigger,started);
		if((s->epo_data_updated == 0)&&gps_epo_wifi_trigger && gps_epo_enable)//if wifi connection,we trigger epo download.
		{
...
CURLcode curl_easy_download_epo(void)
{
    ...
    strcat(gps_epo_data_file_name,"/data/misc/");
...
int mtk_gps_epo_server_data_is_changed()
{
    ...
    if(gps_epo_file_count > 0)
    {
        ...
        strcat(gps_epo_data_file_name_end,"/data/misc/");
    ...
    if((uTime_start - uTime_end) >= (24*60*60))
    {
        ...
        for(i = gps_epo_file_count; 0 <= i;i--)
        {
            ...
            strcat(gps_epo_piece_file_name,"/data/misc/");
