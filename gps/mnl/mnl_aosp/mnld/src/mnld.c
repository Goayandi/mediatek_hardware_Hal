...
void get_time_stamp(char time_str1[], char time_str2[]){
	...
	if(tm_pt)
	{
		sprintf(time_str1, "%d%02d%02d%02d%02d%02d.%1ld", tm_pt->tm_year+1900, tm_pt->tm_mon+1, tm_pt->tm_mday, tm_pt->tm_hour, tm_pt->tm_min, tm_pt->tm_sec,tv.tv_usec/100000);
		sprintf(time_str2, "%d%02d%02d%02d%02d%02d.%03ld", tm_pt->tm_year+1900, tm_pt->tm_mon+1, tm_pt->tm_mday, tm_pt->tm_hour, tm_pt->tm_min, tm_pt->tm_sec,tv.tv_usec/1000);
	}
}
int get_ttff_time(){
	time_t current_time;
...
void get_gps_measurement_clock_data()
{
	...
	if(gps_raw_debug_mode)
	{
		for(i = 0;i < 32;i++)
		{
			if(mtk_gps_measurement[i].size > 0){				
				MND_MSG("i = %d,size = %d, flag = %d,PRN = %d,TimeOffsetInNs = %d, state = %d, ReGpsTowInNs = %d\n",
					i,mtk_gps_measurement[i].size,mtk_gps_measurement[i].flag,mtk_gps_measurement[i].PRN, 
					mtk_gps_measurement[i].TimeOffsetInNs,mtk_gps_measurement[i].state,mtk_gps_measurement[i].ReGpsTowInNs);
				MND_MSG("i = %d,ReGpsTowUnInNs = %d, Cn0InDbHz = %d,PRRateInMeterPreSec = %d,PRRateUnInMeterPreSec = %d, AcDRState10 = %d, AcDRInMeters = %d\n",
					i,mtk_gps_measurement[i].ReGpsTowUnInNs,mtk_gps_measurement[i].Cn0InDbHz,mtk_gps_measurement[i].PRRateInMeterPreSec, 
					mtk_gps_measurement[i].PRRateUnInMeterPreSec,mtk_gps_measurement[i].AcDRState10,mtk_gps_measurement[i].AcDRInMeters);
				MND_MSG("i = %d,AcDRUnInMeters = %d, PRInMeters = %d,PRUnInMeters = %d,CPInChips = %d, CPUnInChips = %d, CFInhZ = %d\n",
					i,mtk_gps_measurement[i].AcDRUnInMeters,mtk_gps_measurement[i].PRInMeters,mtk_gps_measurement[i].PRUnInMeters, 
					mtk_gps_measurement[i].CPInChips,mtk_gps_measurement[i].CPUnInChips,mtk_gps_measurement[i].CFInhZ);			
				MND_MSG("i = %d,CarrierCycle = %d, CarrierPhase = %d,CarrierPhaseUn = %d,LossOfLock = %d, BitNumber = %d, TimeFromLastBitInMs = %d\n",
					i,mtk_gps_measurement[i].CarrierCycle,mtk_gps_measurement[i].CarrierPhase,mtk_gps_measurement[i].CarrierPhaseUn, 
					mtk_gps_measurement[i].LossOfLock,mtk_gps_measurement[i].BitNumber,mtk_gps_measurement[i].TimeFromLastBitInMs);
				MND_MSG("i = %d,DopperShiftInHz = %d, DopperShiftUnInHz = %d,MultipathIndicater = %d,SnrInDb = %d, ElInDeg = %d, ElUnInDeg = %d\n",
					i,mtk_gps_measurement[i].DopperShiftInHz,mtk_gps_measurement[i].DopperShiftUnInHz,mtk_gps_measurement[i].MultipathIndicater, 
					mtk_gps_measurement[i].SnrInDb,mtk_gps_measurement[i].ElInDeg,mtk_gps_measurement[i].ElUnInDeg);
				MND_MSG("i = %d,AzInDeg = %d, AzUnInDeg = %d,UsedInFix = %d\n",
					i,mtk_gps_measurement[i].AzInDeg,mtk_gps_measurement[i].AzUnInDeg,mtk_gps_measurement[i].UsedInFix);						
			}
		}
	}
	...
	if(gps_raw_debug_mode)
	{
		MND_MSG("sizeof(MTK_GPS_CLOCK) = %d, size = %d, flag = %d, leapsecond = %d, type = %d, TimeInNs = %d, TimeUncertaintyInNs = %d, FullBiasInNs = %d, BiasInNs = %d, BiasUncertaintyInNs = %d,DriftInNsPerSec = %d,DriftUncertaintyInNsPerSec = %d",
			sizeof(MTK_GPS_CLOCK), mtk_gps_clock.size, mtk_gps_clock.flag, mtk_gps_clock.leapsecond, mtk_gps_clock.type,mtk_gps_clock.TimeInNs,mtk_gps_clock.TimeUncertaintyInNs, 
			mtk_gps_clock.FullBiasInNs,mtk_gps_clock.BiasInNs,mtk_gps_clock.BiasUncertaintyInNs,mtk_gps_clock.DriftInNsPerSec,mtk_gps_clock.DriftUncertaintyInNsPerSec);
	}
...
void get_gps_navigation_event()
{
	...
	for(svid = 0; svid < 32; svid++)
	{
		...
		if(gps_raw_debug_mode)
		{
				MND_MSG("size = %d,%p, prn = %d,%p, type = %d,%p,message_id = %d,%p, submessage_id = %d,%p, data_length = %d,%p",
					...
					gps_navigation_event->messageID, &gps_navigation_event->messageID, gps_navigation_event->submessageID,
					&gps_navigation_event->submessageID, gps_navigation_event->length, &gps_navigation_event->length);
				for(i = 0;i < 40;i++)
				{					
					MND_MSG("MNLD: gps_navigation_event->data[%d] = %x, %p",i, gps_navigation_event->data[i], &gps_navigation_event->data[i]);		
				}
			}
			...
			MND_MSG("buff_put_struct 'gps_navigation_event' success,sizeof(MTK_GPS_NAVIGATION_EVENT) = %d \n",sizeof(MTK_GPS_NAVIGATION_EVENT));
	        if (-1 == mnld_sendto_hal(mnld2hal_fd, MTK_MNLD2HAL, buff, sizeof(buff)))
...
linux_gps_init (void)
{
	...
	if(strcmp(chip_id, "0x6592") == 0 || strcmp(chip_id, "0x6571") == 0 || strcmp(chip_id, "0x6580") == 0 || strcmp(chip_id, "0x0321") == 0 
		|| strcmp(chip_id, "0x0335") == 0 || strcmp(chip_id, "0x0337") == 0 ||strcmp(chip_id, "0x6735") == 0 || strcmp(chip_id, "0x8163") == 0
		|| strcmp(chip_id, "0x8127") == 0){
		clock_type = ioctl(dsp_fd, 8, NULL);
    	...
    	case 0x02:
		case 0x03:
			MND_MSG("GPS coclock, buffer 2, coVCTCXO\n");
			init_cfg.u1ClockType = 0xFF;
			if(property_set(GPS_CLOCK_TYPE_P, "20") != 0)
				MND_ERR("set GPS_CLOCK_TYPE_P %s\n", strerror(errno));
			break;
		...
		if(strcmp(chip_id, "0x6572") == 0 || strcmp(chip_id, "0x6582") == 0 || strcmp(chip_id, "0x6630") == 0 || strcmp(chip_id, "0x6752") == 0){
...
/* Vanzo:zhangzehui on: Mon, 11 Jan 2016 20:06:36 +0800
 * add VANZO_FEATURE_GPS_CFG
 */
#if defined(VANZO_FEATURE_GPS_CFG_BY_NAME_VALUE)
    if (VANZO_FEATURE_GPS_CFG_BY_NAME_VALUE != NULL
            && strlen((char*)VANZO_FEATURE_GPS_CFG_BY_NAME_VALUE)!= 0) {
        int gpsMode = atoi((char*)VANZO_FEATURE_GPS_CFG_BY_NAME_VALUE);
        if (gpsMode != -1) {
          init_cfg.GNSSOPMode = gpsMode;
        }
    }
#endif
...
static int socket_handler(int sock) //from HAL
{
	...
	char buff[1024] = {0};
	int offset = 0;
	safe_read(sock, buff, sizeof(buff));
   ...
   MND_MSG("args from hal: %d\n", info);
    switch(info) {
        ...
        case MNL_CMD_GPS_INJECT_LOCATION: {
            ...
            MND_MSG("MNL_CMD_GPS_INJECT_LOCATION lati= %f, longi = %f,accuracy =%f,ts.tv_sec= %lld,ts.tv_nsec = %lld\n", nlp_inject.latitude, nlp_inject.longitude,nlp_inject.accuracy,
...
int get_mtklog_path(char *logpath)
{
	...
	memset(raw_path, 0x0, sizeof(raw_path));
	property_get(SDCARD_SWITCH_PROP, raw_path, "");
	MND_MSG("get_mtklog_path:raw path is %s", raw_path);

	memset(temp, 0x0, sizeof(temp));
	if(!strcmp(raw_path,PATH_INTERNAL)) {
		property_get("vold.path.internal_sd", temp, "");
		if(!strlen(temp)) {
			MND_ERR("cant get path through internal_sd_path");
			strcpy(temp, PATH_DEFAULT);
		}
	} else if(!strcmp(raw_path,PATH_EXTERNAL)) {
		property_get("vold.path.external_sd", temp, "");
		if(!strlen(temp)) {
			MND_ERR("cant get path through external_sd_path");
			strcpy(temp, PATH_EX);
		}
	} else {
		strcpy(temp,PATH_DEFAULT);
	...
	memcpy(logpath, temp, len);
	strcat(logpath,PATH_SUFFIX);
	MND_MSG("get_mtklog_path:logpath is %s", logpath);

    memcpy(mtklogpath, temp, len);
	strcat(mtklogpath,"mtklog/");
    ...
    { 
         ...
         if(mkdir(mtklogpath, 0775) == -1){
			 strcpy(logpath,LOG_FILE);//if mkdir fail, set default path 		 
...
int mtklogger2mnl_handler(int fd)
{
	...
	char ans[255] = {0};
	int offset = 0;
	char path[GPS_DEBUG_LOG_FILE_NAME_MAX_LEN] = {0};
	...
	if(!strncmp(buff, "deep_start,1",12)){
		gps_debuglog_state = GPS_DEBUGLOG_ENABLE;
		MND_MSG("gps_debuglog_state:%d",gps_debuglog_state);

		get_mtklog_path(path);	
		strcat(path,"gpsdebug.log");
		strcpy(gps_debuglog_file_name,path);
	...
	for (ne = 0; ne < nevents; ne++) {
		if ((events[ne].events & (EPOLLERR|EPOLLHUP)) != 0) {
			MND_ERR("wait: (%d %d %d %d %d), event: 0x%X from %d", obj->cur_accept_socket,
				obj->sig_rcv_fd, obj->mnl_rcv_agps_fd, obj->mnl_rcv_mtklogger_fd, client_fd, events[ne].events, events[ne].data.fd);
