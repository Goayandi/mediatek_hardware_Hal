...
#ifdef CONFIG_WAPI_SUPPORT
		if (!wapi &&
				( ssid->key_mgmt & (WPA_KEY_MGMT_WAPI_CERT | WPA_KEY_MGMT_WAPI_PSK))
				) {
			  ...
			  int only_first_ssid)
{
	...
	/* for channel conflict revise feature
		if p2p/wfd is connected, search in full scan result to find the ssid. because framework also base on
		this result, but not last scan result. changes in function wpa_bss_in_use will asure the intended bsses
	...
	if (shared_freq > 0) {
		...
		for (i = 0; i < wpa_s->num_bss; i++) {
			...
			*selected_ssid = wpa_scan_res_match(wpa_s, i, temp_bss, group,
			...
			wpa_printf(MSG_DEBUG, "on Freq %d, found a BSS in previous scan res", shared_freq);
			return temp_bss;
		}
		/* it is rarely that we aren't able to find this ssid in full scan result, unless this bss was removed just after 
			framework select this bss and before send command to wpa_supplicant */
		wpa_printf(MSG_ERROR, "didn't find BSS on freq %d, try to find on other freqs", shared_freq);	
	}
#endif /* CONFIG_MTK_WFD_LEGACY_COEX */
			  ...
			  int only_first_ssid)
{
	...
	if (only_first_ssid)
    	...
    	wpa_dbg(wpa_s, MSG_DEBUG, "Selecting BSS from WEP/PSK priority group %d",
	    	group->priority);
	...
	for (i = 0; i < wpa_s->last_scan_res_used; i++) {
		...
		if(rsn_ie) {
			...
			if(!wpa_key_mgmt_wpa_psk(ie_data.key_mgmt)) continue;
		}
		else if(wpa_ie) {
			if(wpa_ie) wpa_parse_wpa_ie(wpa_ie, 2 + wpa_ie[1], &ie_data);
			if(!wpa_key_mgmt_wpa_psk(ie_data.key_mgmt)) continue;
		}
		else {
			if( !(bss->caps & IEEE80211_CAP_PRIVACY) ) continue;
		}
		//end of skip non wep&psk AP

		*selected_ssid = wpa_scan_res_match(wpa_s, i, bss, group, only_first_ssid);
		if (!*selected_ssid)
			continue;
			  ...
			  int only_first_ssid)
{
	...
	if (only_first_ssid)
    	...
    	wpa_dbg(wpa_s, MSG_DEBUG, "Selecting BSS from EAP priority group %d",
	    	group->priority);
	...
	for (i = 0; i < wpa_s->last_scan_res_used; i++) {
		...
		if(rsn_ie) {
			wpa_parse_wpa_ie(rsn_ie, 2 + rsn_ie[1], &ie_data);
			if(!wpa_key_mgmt_wpa_ieee8021x(ie_data.key_mgmt)) continue;
		}
		else if(wpa_ie) {
			if(wpa_ie) wpa_parse_wpa_ie(wpa_ie, 2 + wpa_ie[1], &ie_data);
			if(!wpa_key_mgmt_wpa_ieee8021x(ie_data.key_mgmt)) continue;
			  ...
			  int only_first_ssid)
{
	...
	if (only_first_ssid)
    	...
    	wpa_dbg(wpa_s, MSG_DEBUG, "Selecting BSS from Open priority group %d",
	    	group->priority);
					     ...
					     struct wpa_ssid **selected_ssid)
{
	...
	while (selected == NULL) {
#ifdef CONFIG_MTK_ORANGE
/*ALPS01269970: [6582][wifi][Case Fail]The latest AP's priority is highest 
but not according to the priority "WPA/WPA2 PSK>WEP>802.1X>OPEN". */
        for (prio = 0; prio < wpa_s->conf->num_prio; prio++) {
            if (next_ssid && next_ssid->priority ==
		        wpa_s->conf->pssid[prio]->priority) {
				selected = wpa_supplicant_select_bss_weppsk(
        ...
        if(!selected) {
            for (prio = 0; prio < wpa_s->conf->num_prio; prio++) {
                if (next_ssid && next_ssid->priority ==
			        wpa_s->conf->pssid[prio]->priority) {
    				selected = wpa_supplicant_select_bss_eap(
    					wpa_s, next_ssid, selected_ssid, 1);
    				if (selected)
    					break;
			    }
        ...
        if(!selected) {
            for (prio = 0; prio < wpa_s->conf->num_prio; prio++) {
                if (next_ssid && next_ssid->priority ==
			        wpa_s->conf->pssid[prio]->priority) {
    				selected = wpa_supplicant_select_bss_open(
    					wpa_s, next_ssid, selected_ssid, 1);
    				if (selected)
    					break;
			    }
					  ...
					  union wpa_event_data *data)
{
	...
	if (data->assoc_info.freq) {
		...
		shared_freq = wpa_drv_shared_freq(wpa_s);
		if (shared_freq > 0 &&
			data->assoc_info.freq != shared_freq) {

			if (os_strcmp(wpa_s->ifname, "wlan0")) {
				wpa_printf(MSG_DEBUG, "MTK_SCC: p2p has connected, channel conflict wlan %d != p2p %d",
							data->assoc_info.freq, shared_freq);
				/* case: P2P is connected, Legacy channel conflict */
				wpa_s->global->p2p->peer_oper_freq = shared_freq;
			} else {
				wpa_printf(MSG_DEBUG, "MTK_SCC: wlan has connected, channel conflict wlan %d != p2p %d",
								shared_freq, data->assoc_info.freq);
				/* case: Legacy is connected, p2p channel conflict */
				wpa_s->global->p2p->peer_oper_freq = data->assoc_info.freq;
			}
		}
	}
#else
					  ...
					  int locally_generated)
{
	...
	if (!is_zero_ether_addr(bssid) ||
	    wpa_s->wpa_state >= WPA_AUTHENTICATING) {
#ifdef CONFIG_MTK_SCC
		if (reason_code == WLAN_REASON_DEAUTH_FREQ_CONFLICT)
						 ...
						 int locally_generated)
{
	...
	if (!wpa_s->disconnected &&
		...
		if (wpa_s->wpa_state == WPA_COMPLETED &&
			 ...
			 */
/*			
[ALPS00397927] Wifi-direct test: Group joint test fail randomly by CTS verifier tool
[WLAN][WPA_SUPPLICANT]
1. Fix CTS test problem about P2P connection.
    a. Set listen channel from GO negotiation Request. This patch can fix wrong channel for unkown peer invitation.
    b. Reject auto-connection if GROUP-REMOVE command comes from user's request
    c. Set operating channel from GO negotiation Confirmation to prevent from wrong channel operation
2. Fix CTS test problem about GROUP join.
    a. Remove WSC procedure if GO is not ready.
    b. Cancel group idle timer if GROUP Remove remove comes from user
Review: http://mtksap20:8080/go?page=NewReview&reviewid=50144
*/ 
	...
	if (
		...
		wpa_s->key_mgmt != WPA_KEY_MGMT_WAPI_PSK &&
#endif
		wpa_supplicant_dynamic_keys(wpa_s)) {
