				 ...
				 enum p2p_group_removal_reason removal_reason)
{
...
#ifdef CONFIG_MTK_SCC /* code for p2p wifi vs legacy wifi channel conflict to pop up dialog */
		if (removal_reason == P2P_GROUP_REMOVAL_FREQ_CONFLICT)
			wpa_supplicant_deauthenticate(wpa_s,
					      	WLAN_REASON_DEAUTH_FREQ_CONFLICT);
		else
#endif
	...
	if (removal_reason != P2P_GROUP_REMOVAL_SILENT) {
		...
		if (removal_reason == P2P_GROUP_REMOVAL_FREQ_CONFLICT)
			wpa_msg_global(wpa_s->parent, MSG_INFO,
				       P2P_EVENT_GROUP_REMOVED "%s %s%s peer_oper_freq=%d",
				       wpa_s->ifname, gtype, reason, wpa_s->global->p2p->peer_oper_freq);
		else
			wpa_msg_global(wpa_s->parent, MSG_INFO,
				     ...
				     const u8 *ssid, size_t ssid_len)
{
	...
	if (wpa_s->go_params && is_zero_ether_addr(bssid))
		bssid = wpa_s->bssid;
#endif
...
static void wpas_go_neg_completed(void *ctx, struct p2p_go_neg_results *res)
{
	...
	if (res->status) {
#ifdef CONFIG_MTK_SCC
		if (res->status == P2P_SC_FAIL_NO_COMMON_CHANNELS)
			wpa_msg_global(wpa_s, MSG_INFO,
					   P2P_EVENT_GO_NEG_FAILURE "status=%d peer_oper_freq=%d",
					   res->status, wpa_s->global->p2p->peer_oper_freq);
		else
			wpa_msg_global(wpa_s, MSG_INFO,
			   ...
			   int new_device)
{
	...
	if (wpa_s->p2p_scan_res_cache) {
		/* flash p2p devices to /data/misc/wifi/p2p_devices.cache */
#define P2P_CACHE_FILE "/data/misc/wifi/p2p_devices.cache"
		wpa_printf(MSG_DEBUG, "Flash found p2p devices to %s", P2P_CACHE_FILE);
		FILE *f;
		struct p2p_data *p2p;
		struct p2p_device *dev;
		f = fopen(P2P_CACHE_FILE, "w");
		if (f == NULL) {
			wpa_printf(MSG_DEBUG, "Failed to open '%s' for writing", P2P_CACHE_FILE);
			goto done;
		}
		p2p = wpa_s->global->p2p;
		dl_list_for_each(dev, &p2p->devices, struct p2p_device, list) {
			fprintf(f, "p2p_device={\n");
			fprintf(f, "\taddr=" MACSTR "\n", MAC2STR(dev->info.p2p_device_addr));
			fprintf(f, "\tp2p_dev_addr=" MACSTR "\n", MAC2STR(dev->info.p2p_device_addr));
			fprintf(f, "\tpri_dev_type=%s\n", wps_dev_type_bin2str(dev->info.pri_dev_type, devtype,
												sizeof(devtype)));
			fprintf(f, "\tname='%s'\n", dev->info.device_name);
			fprintf(f, "\tconfig_methods=0x%x\n", dev->info.config_methods);
			fprintf(f, "\tdev_capab=0x%x\n\tgroup_capab=0x%x\n", dev->info.dev_capab,
...
#define WFD_DEV_INFO_SIZE 9
			...
			fprintf(f, "\twfd_dev_info=%s%s", wfd_dev_info_hex[0] ? "0x" : "", wfd_dev_info_hex);
#endif /* CONFIG_WIFI_DISPLAY */
			fprintf(f, "\n");
			fprintf(f, "}\n");
		}
				     ...
				     int op_freq)
{
...
#ifdef CONFIG_MTK_P2P /* [ALPS00618361] [WFD Quality Enhancement] [changelist 1686130] */
    ...
    if (dev != NULL) {
        ...
        wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_INVITATION_RECEIVED "sa=" MACSTR
    		" go_dev_addr=" MACSTR
            " wfd_en=0", MAC2STR(sa), MAC2STR(go_dev_addr));
        }
#else
	...
	if (status == P2P_SC_FAIL_NO_COMMON_CHANNELS) {
		if (bssid) {
			wpa_msg_global(wpa_s, MSG_INFO, P2P_EVENT_INVITATION_RESULT
					   "status=%d " MACSTR " peer_oper_freq=%d",
					   status, MAC2STR(bssid), wpa_s->global->p2p->peer_oper_freq);
		} else {
			wpa_msg_global(wpa_s, MSG_INFO, P2P_EVENT_INVITATION_RESULT
					   "status=%d peer_oper_freq=%d", status, wpa_s->global->p2p->peer_oper_freq);
		}
	} else {
		if (bssid) {
			wpa_msg_global(wpa_s, MSG_INFO, P2P_EVENT_INVITATION_RESULT
...
#ifdef CONFIG_MTK_P2P_5G	
	{ HOSTAPD_MODE_IEEE80211A, 125, 165, 169, 4, BW20 },
#endif
				   ...
				   struct wpa_scan_results *scan_res)
{
	...
	if (freq > 0) {
		...
		if (wpas_check_freq_conflict(wpa_s, freq) > 0) {
			...
			wpa_msg_global(wpa_s->parent, MSG_INFO,
					   P2P_EVENT_GROUP_FORMATION_FAILURE
					   "reason=FREQ_CONFLICT peer_oper_freq=%d", freq);
#else
			wpa_msg_global(wpa_s->parent, MSG_INFO,
				       P2P_EVENT_GROUP_FORMATION_FAILURE
...
#ifdef CONFIG_MTK_SCC /* Config: the legacy & p2p must work on the single channel */
...
#ifdef CONFIG_MTK_SCC_MCC
	if (p2p->p2p_use_mcc) {
		wpa_printf(MSG_DEBUG, "P2P: Use MCC to build connection");
		force_freq = 0;
		pref_freq = 0;
	}
#endif
...
#ifdef CONFIG_MTK_P2P_5G
	if (wpa_s->global->p2p->inv_op_freq) {
		wpa_printf(MSG_DEBUG, "P2P: Select GO oper channel base on invitation res %d",
			wpa_s->global->p2p->inv_op_freq);
		return wpa_s->global->p2p->inv_op_freq;
	} else
		return freq;
#else
	return freq;
...
#ifdef CONFIG_MTK_P2P_5G
	/* only scan the GO oper channel */
	int *scan_req = os_calloc(2,sizeof(int));
	*scan_req = wpa_s->global->p2p->inv_op_freq;
	wpa_s->next_scan_freqs = scan_req;
	wpa_printf(MSG_DEBUG, "P2P: Only scan GO oper chanenl %d", 
					*scan_req);
	
	/* sleep 200ms to wait GO setup */
	wpa_printf(MSG_DEBUG, "P2P: Sleep 300ms to wait GO setup");
	os_sleep(0, 300000);
#endif
	...
	eloop_register_timeout(P2P_MAX_INITIAL_CONN_WAIT, 0,
			       ...
			       wpa_s->parent, NULL);
#ifdef CONFIG_MTK_SCC
	/* ALPS01771578 Flush Bss here, supplicant will re-scan before connect */
	wpa_bss_flush(wpa_s);
#endif
...
/*
[ALPS00326459] [6577JB][CTS Verifier 4.1_r1][MT6577_PHONE][changelist 989498]
...
int wpas_p2p_cancel(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->connect_work) {
		wpa_printf(MSG_DEBUG, "P2P: Remove connect work to avoid race condition");	
...
static void wps_p2p_beamplus_go_reserve_timeout(void *eloop_ctx, void *timeout_ctx)
{
	struct p2p_data *p2p = eloop_ctx;

	wpa_printf(MSG_DEBUG, "P2P: Beamplus GO reserve timeout");
	p2p->in_beam_go_reserve = 0;

}

void wpas_p2p_beamplus_go_reserve_register_timeout(struct wpa_supplicant *wpa_s)
{
	struct p2p_data *p2p;

	p2p = wpa_s->global->p2p;

	eloop_cancel_timeout(wps_p2p_beamplus_go_reserve_timeout, p2p, NULL);
	eloop_register_timeout(20, 0, wps_p2p_beamplus_go_reserve_timeout,
			       p2p, NULL);
}

static void wps_p2p_beamplus_go_ready_timeout(void *eloop_ctx, void *timeout_ctx)
{
	struct wpa_supplicant *wpa_s = eloop_ctx;
	static timeout_cnt = 0;
	int buf = 0;

	wpa_printf(MSG_DEBUG, "P2P: Beamplus polling GO ready");
	timeout_cnt++;
	wpa_drv_driver_cmd(wpa_s, "get_beamplus_go_ready", (char *)&buf, sizeof(int));
	if (buf == 1)
		// Notify 
		wpa_msg(wpa_s, MSG_INFO, "P2P-GO-SCAN-DONE");
	else {
		if (timeout_cnt < 10) { 
			wpas_p2p_beamplus_go_ready_register_timeout(wpa_s);
		} else {
			timeout_cnt = 0;
		}
	}	
...
void wpas_p2p_beamplus_go_ready_register_timeout(struct wpa_supplicant *wpa_s)
{

	eloop_cancel_timeout(wps_p2p_beamplus_go_ready_timeout, wpa_s, NULL);
	eloop_register_timeout(1, 0, wps_p2p_beamplus_go_ready_timeout,
			       wpa_s, NULL);
}
...
#ifdef CONFIG_MTK_WPS_P2P_NFC
void *wpas_p2p_wps_nfc_token_gen( int *id, struct wpabuf **pubkey,
					  ...
					  int ndef, int tag)
{
	...
	if (!tag && wpa_s->conf->wps_nfc_dh_pubkey == NULL &&
		...
		return NULL;
#ifdef CONFIG_MTK_WPS_P2P_NFC
	if (tag && wpa_s->conf->wps_nfc_dev_pw == NULL) {
		struct wpabuf *buf = NULL;
		struct wpa_supplicant *iface;

		wpa_printf(MSG_DEBUG, "P2P: Build WPS NFC TOKEN for P2P SEL TAG");
		wpas_p2p_wps_nfc_token_gen(&wpa_s->conf->wps_nfc_dev_pw_id,
				 &wpa_s->conf->wps_nfc_dh_pubkey,
				 &wpa_s->conf->wps_nfc_dh_privkey,
				 &wpa_s->conf->wps_nfc_dev_pw);
		p2p_set_authorized_oob_dev_pw_id(
			wpa_s->global->p2p, wpa_s->conf->wps_nfc_dev_pw_id, wpa_s->conf->p2p_go_intent,
			wpa_s->own_addr);
		...
		wpa_s->p2p_oob_dev_pw = wpabuf_dup(wpa_s->conf->wps_nfc_dev_pw);
			if (wpa_s->p2p_oob_dev_pw == NULL)
				return NULL;			
		wpa_s->p2p_nfc_tag_enabled = 1;				
		// Add P2P NFC token for GO interface				
		for (iface = wpa_s->global->ifaces; iface; iface = iface->next) {
			struct hostapd_data *hapd;
			if (iface->ap_iface == NULL)
				continue;
...
#endif
#ifdef CONFIG_MTK_SCC
int wpas_get_peer_oper_freq(struct wpa_supplicant *wpa_s)
{
	return wpa_s->global->p2p->peer_oper_freq;
}
#endif
