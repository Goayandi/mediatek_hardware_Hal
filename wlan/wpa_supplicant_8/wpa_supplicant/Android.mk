...
ifeq ($(MTK_WIFIWPSP2P_NFC_SUPPORT), yes)
CONFIG_MTK_WPS_P2P_NFC=y
endif
...
$(warning set CONFIG_MEDIATEK)
L_CFLAGS += -DCONFIG_MEDIATEK
#CONFIG_MTK_P2P support MTK extended P2P feature/bugfix
ifdef CONFIG_MTK_P2P
L_CFLAGS += -DCONFIG_MTK_P2P
endif

ifdef CONFIG_MTK_DUMP_KEY
L_CFLAGS += -DCONFIG_MTK_DUMP_KEY
endif

#CONFIG_MTK_P2P_SIGMA, MTK P2P sigma tool control switch
ifdef CONFIG_MTK_P2P_SIGMA
L_CFLAGS += -DCONFIG_MTK_P2P_SIGMA
endif

ifdef CONFIG_MTK_SCC
$(warning value of MTK_WIFI_MCC_SUPPORT $(MTK_WIFI_MCC_SUPPORT))
ifeq ($(MTK_WIFI_MCC_SUPPORT), yes)
L_CFLAGS += -DCONFIG_MTK_SCC_MCC
endif
#CONFIG_MTK_SCC always configured
L_CFLAGS += -DCONFIG_MTK_SCC
endif
#When WFD and Legacy coexist, do some revisions for Mediatek Platform
L_CFLAGS += -DCONFIG_MTK_WFD_LEGACY_COEX
L_CFLAGS += -DCONFIG_MTK_PRESET_SSID
...
ifeq ($(MTK_WIFI_CHINESE_SSID),yes)
L_CFLAGS += -DCONFIG_CHINESE_SSID
endif
...
L_CFLAGS += -DCONFIG_MTK_WPS_P2P_NFC
endif
...
L_CFLAGS += -DCONFIG_RILD_FUNCS_MTK_DT
endif #MTK_DT_SUPPORT
ifeq ($(EVDO_DT_SUPPORT), yes)
L_CFLAGS += -DCONFIG_RILD_FUNCS_EVDO_DT
...
L_CFLAGS += -DCONFIG_WAPI_CERT_FROM_KEYSTORE
endif

#if WPAPSK is not supported, exclude WPAPSK flag from WPS M1 and M2 message
L_CFLAGS += -DCONFIG_MTK_WPAPSK

#MTK's driver always scan wildcard ssid even set the specified ssid for hidden ssid ap.
#so we don't trigger another wildcard scan in supplicant
L_CFLAGS += -DCONFIG_MTK_HIDDEN_SSID

#When configuration file are being writed then pull out battery quickly, 
#Configuration file will be flushed after restart
L_CFLAGS += -DCONFIG_MTK_WRITE_FILE

ifeq ($(MTK_TC1_FEATURE), yes)
L_CFLAGS += -DCONFIG_ALWAYS_SPECIFIC_SCAN
endif

ifeq ($(MTK_WIFI_GET_IMSI_FROM_PROPERTY), yes)
L_CFLAGS += -DCONFIG_GET_IMSI_FROM_PROPERTY
endif

ifeq ($(OPTR_SPEC_SEG_DEF), OP01_SPEC0200_SEGC)
L_CFLAGS += -DCONFIG_MTK_CMCC
endif
...
#CONFIG_MTK_STAGE_SCAN=y
ifdef CONFIG_MTK_STAGE_SCAN
L_CFLAGS += -DCONFIG_MTK_STAGE_SCAN
endif
