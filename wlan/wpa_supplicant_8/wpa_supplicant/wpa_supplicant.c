...
static void wpas_start_assoc_cb(struct wpa_radio_work *work, int deinit)
{
	...
	params.mgmt_frame_protection =
		ssid->ieee80211w == MGMT_FRAME_PROTECTION_DEFAULT ?
		wpa_s->conf->pmf : ssid->ieee80211w;
#if 0 /* let driver to decide the pmf req or cap, for PMF certification */
...
#ifdef CONFIG_WAPI_SUPPORT
	if((wpa_s->key_mgmt == WPA_KEY_MGMT_WAPI_PSK)||(wpa_s->key_mgmt == WPA_KEY_MGMT_WAPI_CERT)){
	...
	if (wpa_s->num_multichan_concurrent < 2) {
		int freq, num;
#ifdef CONFIG_MTK_SCC
		num = 1; /* always make this variable greater than 0, because we don't care it if MTK_SCC defined */
...
#ifdef CONFIG_MTK_SCC
			if (os_strncmp(wpa_s->ifname, "p2p0", os_strlen("p2p0")) == 0)
				/* case: Legacy is connected, p2p channel conflict */
				wpa_s->global->p2p->peer_oper_freq = params.freq.freq;
			else
				/* case: P2P is connected, legacy channel conflict */
				wpa_s->global->p2p->peer_oper_freq = freq;
#endif
...
#ifdef CONFIG_WAPI_SUPPORT
	    if(wpa_s->key_mgmt & (WPA_KEY_MGMT_WAPI_PSK|WPA_KEY_MGMT_WAPI_CERT)){
...
/*ALPS01841137: [Side Effect][K2 64][LMP][TK][WiFi]Can't connect to AP when WFD is connected.*/
		if (radio_work_pending(wpa_s, "scan") ||
