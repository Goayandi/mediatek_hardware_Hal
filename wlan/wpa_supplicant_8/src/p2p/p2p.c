		   ...
		   size_t ies_len, int scan_res)
{
	...
	dev_capab_orig = dev->info.dev_capab;
	...
	if (msg.wfd_subelems) {
		...
		* or WFD information has changed
		*/
		if (!dev->info.wfd_subelems ||
			os_memcmp(dev->info.wfd_subelems, msg.wfd_subelems,
...
// Peter: add for Beamplus application
	if ((type == P2P_SCAN_SOCIAL) && (p2p->in_beam_scan == 1)) {
		type = P2P_SCAN_FULL;
		wpa_printf(MSG_DEBUG, "P2P: P2P_SEARCH use Beam Plus");
	}
...
// Peter: add for Beamplus application
	if (timeout == 123) {
		p2p->in_beam_scan = 1;
		wpa_printf(MSG_DEBUG, "P2P: Beam Plus Scan invoke");		
	} else
		p2p->in_beam_scan = 0;
// Peter: end of Beamplus application
#endif /*Mediatek Modified*/
...
#else
		p2p_dbg(p2p, "Forget Force Channel!");
#endif
	else
		dev->flags &= ~P2P_DEV_FORCE_FREQ;
...
#ifdef CONFIG_MTK_P2P
		if (dev->flags & P2P_DEV_INVITE_SCHEDULE) {
			p2p_dbg(p2p, "Pending Invitation request, send it out again");
...
static void p2p_prov_disc_cb(struct p2p_data *p2p, int success)
{
	...
	if (!success) {
		 ...
		 */
		/* ALPS01437544 */
		wpa_printf(MSG_DEBUG, "PD Request send failed, trigger the p2p find phase");
		...
		if (p2p->invite_peer) {
			...
			if (p2p->cfg->invitation_result)
				p2p->cfg->invitation_result(
					...
					0, 0);
#ifdef CONFIG_MTK_P2P
			p2p_dbg(p2p, "Mark off p2p pending invitation flag");
			p2p->invite_peer->flags &= ~P2P_DEV_INVITE_SCHEDULE;
#endif
		}
		p2p_set_state(p2p, P2P_IDLE);
	}
...
static void p2p_state_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	p2p->in_listen = 0;
#ifdef CONFIG_MTK_P2P
	/* sync from hostap devel community. Commit ID: a902d */
	if (p2p->drv_in_listen ||
	    radio_work_pending(wpa_s, "p2p-listen")) {
		p2p_dbg(p2p, "Driver is still in listen state or have listen radio work - stop it");
		p2p->cfg->stop_listen(p2p->cfg->cb_ctx);
	}
#endif
            ...
            if(msg->wfd_subelems && msg->wfd_device_info){
                ...
                if((p2p->wfd_params->WfdDevInfo & 0x3) == 0x3) {
                    ...
                    if((u2DevInfo &0x3) == 0x00 /* Peer is Source*/) {
                        ...
                        wpa_drv_driver_cmd(p2p->cfg->cb_ctx, "wfd_data_update", 
											(char *)(p2p->wfd_params), sizeof(struct wfd_data_s));
               ...
               if((p2p->wfd_params->WfdDevInfo & 0x3) == 0x0) {
                     p2p_dbg(p2p, WFD_EVENT_AUTO_SWITCH_ROLE "role=0");
               }
               else if((p2p->wfd_params->WfdDevInfo & 0x3) == 0x1) {
                     p2p_dbg(p2p, WFD_EVENT_AUTO_SWITCH_ROLE "role=1");
               }
               else {
