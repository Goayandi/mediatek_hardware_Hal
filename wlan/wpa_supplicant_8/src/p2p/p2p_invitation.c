				...
				const u8 *data, size_t len, int rx_freq)
{
	...
	if (op_freq) {
		...
		if (!p2p_channels_includes(&intersection, reg_class, channel))
		{
			...
			p2p_dbg(p2p, "Reselect channel in invitation context");
				 ...
				 const u8 *data, size_t len)
{
	...
	if (msg.operating_channel) {
		p2p->peer_oper_freq = p2p_channel_to_freq(
    						     msg.operating_channel[3],
    						     msg.operating_channel[4]);	
		p2p_dbg(p2p, "%s peer oper channel from invitation resp %d", __func__, p2p->peer_oper_freq);
	}
#endif
	...
	if (!msg.channel_list && *msg.status == P2P_SC_SUCCESS) {
		...
		wpa_msg_global(wpa_s, MSG_INFO, P2P_EVENT_INVITATION_RESULT
				   "status=7 peer_oper_freq=%d", wpa_s->global->p2p->peer_oper_freq);
#endif
		return;
	} else {
...
#ifdef CONFIG_MTK_P2P
	if (!success) {
	/* Invitation Request TX failed, peer maybe on different channels due to switch to
	 * p2p find phase, trigger p2p find phase to update peer device listen channel,
	 * this will update offchannel TX freq
	 */
		p2p_dbg(p2p, "Invitation Request failed, trigger p2p find phase");
		p2p->invite_peer->flags |= P2P_DEV_INVITE_SCHEDULE;
