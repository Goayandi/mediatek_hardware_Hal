...
#if defined (CONFIG_MTK_SCC) || defined (CONFIG_MTK_WFD_SINK)
	u8 no_common_ch = 0;
...
#if defined (CONFIG_MTK_SCC) || defined (CONFIG_MTK_WFD_SINK)
	p2p_channels_dump(p2p, "nego req prepared channels", &p2p->channels);

	if (p2p->channels.reg_classes != 0 && msg.channel_list &&
			p2p_peer_channels_check(p2p, &p2p->channels, dev,
						   msg.channel_list,
						   msg.channel_list_len) < 0) {
			if (msg.operating_channel)
				p2p->peer_oper_freq = p2p_channel_to_freq(msg.operating_channel[3],
									 msg.operating_channel[4]);

			p2p_dbg(p2p, "Really has no common channels; Store peer oper freq from nego req: %d MHz",
				p2p->peer_oper_freq);
			no_common_ch = P2P_SC_FAIL_NO_COMMON_CHANNELS;
	}
#endif
...
*/			
...
#if defined (CONFIG_MTK_SCC) || defined (CONFIG_MTK_WFD_SINK)
		if (no_common_ch == 7) {
			wpa_printf(MSG_DEBUG, "report to fwk status=7");
			wpa_msg_global(wpa_s, MSG_INFO, P2P_EVENT_GO_NEG_FAILURE "status=7 peer_oper_freq=%d",
					p2p->peer_oper_freq);
		}
#endif
		...
		if (p2p_peer_channels(p2p, dev, msg.channel_list,
			...
			if (msg.operating_channel) {
				p2p->peer_oper_freq= p2p_channel_to_freq(
									 msg.operating_channel[3],
									 msg.operating_channel[4]);
				p2p_dbg(p2p, "P2P: Peer operating from P2P nego req"
					"channel preference: %d MHz in GO Req", p2p->peer_oper_freq);
			}
#endif
			goto fail;
		}
...
#ifdef CONFIG_MTK_P2P
    	if (msg.operating_channel) {
    		dev->oper_freq = p2p_channel_to_freq(
    						     msg.operating_channel[3],
    						     msg.operating_channel[4]);
    		p2p_dbg(p2p, "P2P: Peer operating "
			     ...
			     const u8 *data, size_t len, int rx_freq)
{
	...
	if (*msg.status) {
		...
		if (*msg.status == P2P_SC_FAIL_INFO_CURRENTLY_UNAVAILABLE) {
			...
			p2p_set_timeout(p2p, 0, 0);
#ifdef CONFIG_MTK_SCC_MCC
			/* check if has no common channels  */
			if (msg.channel_list && p2p_peer_channels(p2p, dev, msg.channel_list,
					  msg.channel_list_len) < 0) {
				p2p_dbg(p2p, "%s bring back all channels", __func__);
				os_memcpy(&p2p->channels, &p2p->cfg->channels,
					  sizeof(struct p2p_channels));
				if (msg.operating_channel) {
					p2p->peer_oper_freq = p2p_channel_to_freq(msg.operating_channel[3],
	...
	if (p2p_peer_channels(p2p, dev, msg.channel_list,
		...
		if (msg.operating_channel) {
			...
			p2p_dbg(p2p, "Store peer oper freq from nego resp: %d MHz",
				p2p->peer_oper_freq);
		}
#ifdef CONFIG_MTK_SCC_MCC
		if (p2p_peer_channels_check(p2p, &p2p->cfg->channels, dev,
							   msg.channel_list, msg.channel_list_len) < 0) {
			p2p_dbg(p2p, "Nego response: Really has no common channel!");
			status = P2P_SC_FAIL_NO_COMMON_CHANNELS;
			goto fail;
		} else {
			p2p_dbg(p2p, "%s %d bring back all channels", __func__, __LINE__);
			os_memcpy(&p2p->channels, &p2p->cfg->channels,
				  sizeof(struct p2p_channels));
			wpa_msg_global(wpa_s, MSG_INFO, P2P_EVENT_GO_NEG_FAILURE "status=7 peer_oper_freq=%d",
									p2p->peer_oper_freq);
